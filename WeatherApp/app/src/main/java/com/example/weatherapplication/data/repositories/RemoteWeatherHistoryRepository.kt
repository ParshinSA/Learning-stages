...
class RemoteWeatherHistoryRepository {
    suspend fun requestHistoryMonth(cityId: Int, month: Int): HistoryData {
        return Networking.historyApi.requestStatisticalMonthlyAggregation(cityId, month).result
    }

    suspend fun requestHistoryDay(cityId: Int, month: Int, day: Int): HistoryData {
        return Networking.historyApi.requestStatisticalDailyAggregation(cityId, month, day).result
    }
}